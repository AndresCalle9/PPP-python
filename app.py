...
# def getdatas():
#     return jsonify({"datas": datas, "message": "Lista de datos"})

# @app.route('/datas/<string:data_name>')
# def getData(product_name):
#     dataFound = [data for data in datas if data['id_user'] == data_name]
#     if (len(dataFound) > 0):
#         return jsonify({"data": dataFound[0]})
#     return jsonify({"message": "dato no encontrado"})
...
def addData():
    ...
    #     dicjs = json.dumps(signalBase, indent=4)
    #     file.write(dicjs)    
    ...
    # with open("ampN.json", "w") as file:
    #     dicjs = json.dumps(ampN, indent=4)
    #     file.write(dicjs)

    # with open("phaN.json", "w") as file:
    #     dicjs = json.dumps(phaN, indent=4)
    #     file.write(dicjs)

    return jsonify({"amp" : ampData, "pha" : phaData, "addInfo" : addInfo, "signalBase" : signalBase})

# @app.route("/datas/<string:data_name>", methods = ["PUT"])
# def updateData(data_name):
#     dataFound = [data for data in datas if data['id_user'] == data_name]
#     if (len(dataFound) > 0):
#         dataFound[0]["id_user"] = request.json["id_user"]
#         dataFound[0]["id_little"] = request.json["id_little"]
#         dataFound[0]["date"] = request.json["date"]
#         return jsonify({"data": dataFound[0], "message":"Dato actualizado"})
#     return jsonify({"message": "Dato no encontrado"})

# @app.route("/datas/<string:data_name>", methods = ["DELETE"])
# def deleteData(data_name):
#     dataFound = [data for data in datas if data['id_user'] == data_name]
#     if (len(dataFound) > 0):
#         datas.remove(dataFound[0])
#         return jsonify({"data": datas, "message":"Dato eliminado"})
#     return jsonify({"message": "dato no encontrado"})
